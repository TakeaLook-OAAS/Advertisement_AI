...
class GazeDetector:
    ...
    def detect_batch(self, frame: np.ndarray, tracks: List[Track]) -> List[Track]:
        """
        여러 track의 눈 crop + headpose를 모아 한 번의 배치 추론으로 gaze를 구합니다.
        """
        size = GazeNet.EYE_SIZE
        ...
        for track in tracks:
            ...
            left_crops.append(cv2.resize(left_crop, (size, size)))
            right_crops.append(cv2.resize(right_crop, (size, size)))
            hp_vecs.append([hp.yaw / 90.0, hp.pitch / 90.0, hp.roll / 90.0])

        if not valid_tracks:
            return tracks

        left_t = self._to_batch_tensor(left_crops)
        right_t = self._to_batch_tensor(right_crops)
        hp_t = torch.tensor(hp_vecs, dtype=torch.float32, device=self.device)

        with torch.no_grad():
            gaze_out = self.model(left_t, right_t, hp_t)

        for track, gaze in zip(valid_tracks, gaze_out):
            track.gaze = Gaze(x=float(gaze[0]), y=float(gaze[1]), z=float(gaze[2]))
